...
export default function Schedule() {
  ...
  const [commandBarValue, setCommandBarValue] = useLocalStorage("commandBarValue", "");
  const [rowData, setRowData] = useLocalStorage("rowData", []);
  const [rowCount, setRowCount] = useLocalStorage("rowCount", 0);
  ...
  const updateCommandBar = () => {
    console.log("updateCommandBar")
    const commandArgs = rowData
      .map((data, index) => {
        if (data.checked) {
    ...
    console.log(teuthologySuiteCommand);
    setCommandBarValue(teuthologySuiteCommand);
  };
